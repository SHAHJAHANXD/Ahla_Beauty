...
class SalonController extends Controller
{
    ...
    {
        $time = Available::where('salon_id', $id)->get();
        if ($time == true) {
            $response = ['status' => true, 'data' => $time, 'message' => "Data fetched successfully. Thank you!"];
    ...
    public function available(Request $request)
    {
        $validator = Validator::make(
            ...
            [
                ...
                'month_date.required' => 'Date is required!',
            ]
        );
        if ($validator->fails()) {
        ...
        $time = new Available();
        $time->salon_id = Auth::user()->id;
        $time->time_slot = $request->time_slot;
        $time->month_date = $request->month_date;
        $time->save();
        if ($time == true) {
            $response = ['status' => true, 'data' => $time, 'message' => "Record saved successfully. Thank you!"];
            return response($response, 200);
        } else {
            $response = ['status' => false, 'data' => null, 'message' => "Something went wrong. Thank you!"];
            return response($response, 400);
        }
    }
